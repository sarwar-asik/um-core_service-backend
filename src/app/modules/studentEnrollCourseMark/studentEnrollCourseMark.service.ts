...
const updateFinalMarks = async (payload: any) => {
  ...
  if (!studentEnrollCourse) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'studentEnrolledCourse data not found'
    );
  }
  const studentEnrollCourseMarks =
  ...
  if (!studentEnrollCourseMarks?.length) {
    throw new ApiError(
      httpStatus.BAD_REQUEST,
      'studentEnrolledCourseMarks data not found'
    );
  }
  ...
  const totalFinalMarks =
    Math.ceil(midTermMarks * 0.4) + Math.ceil(finalTermMarks * 0.6); ///math used for round figure marks

  console.log('🚀 totalFinalMarks:', totalFinalMarks);

  const result =
    studentEnrollCourseMarkUtils.getGradeFromMarks(totalFinalMarks);
  ...
  const academicResult = await studentEnrollCourseMarkUtils.calcCGPAandGrade(
    grades
  );

  const studentAcadeInfo = await prisma.studentAcademicInfo.findFirst({
    where: {
      student: {
        id: studentId,
      },
